...
export class Header implements Component {
  ...
  <path fill-rule="evenodd" clip-rule="evenodd" d="M0 110L10 40L50 0L100 50L70 80L110 120L50 160L0 110Z" fill="#00D1B2"/>
    ...
    navbarWrapper.classList.add('navbar-menu');

    const itemWrapper = document.createElement('div');
    itemWrapper.classList.add('navbar-start');

    navbarWrapper.append(itemWrapper);

    this.headerWrapper.append(navbarWrapper);

    const isLogged = accessTokenStorage.hasValue();
    ...
    if (isLogged) {
      loggedLinks.forEach(({ title, path }) => {
      ...
      unloggedLinks.forEach(({ title, path }) => {
        const link = document.createElement('a');
        link.classList.add('navbar-item');
        link.textContent = title;
        link.href = path;
